...
namespace LittleGameplayTweaks
{
    public class DCCS
    {
        ...
        {
            if (category.cards.Length >= start + 2)
            {
                //Double check for mods that change the order or have random dccs or whatever
                if (category.cards[start + 1].spawnCard.name.EndsWith("tHealing") &&
                    category.cards[start + 2].spawnCard.name.EndsWith("tUtility"))
                {
